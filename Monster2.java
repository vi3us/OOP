public class Monster2 extends Monster
{
    ...
    public Monster2(int hp, int maxHp, int speed, int money)
    {
        super(hp, maxHp, speed, money, "Cerberus");
    }    
    public String fight(Monster wildMonster, Player player1){
        ...
        if(wildMonster.getMonsterType().equals("Ghoul")){
            Monster3 m3 = (Monster3)wildMonster;
            tempString += ("your monster is naturally stronger than Ghoul" + "\n");
            setStrengthConstant(2);
            fightConstant = 3;
            calculateFightOutcome();
            ///m3.ifRegenHp();
            tempString += m3.getTempString();
        }else if(wildMonster.getMonsterType().equals("Minotaur")){
            Monster1 m1 = (Monster1)wildMonster;
            tempString += "your monster is naturally weaker than Minotaur\n";
            setStrengthConstant(-2);
            fightConstant = 1;
            if (m1.getEscape()==false){
                tempString += m1.getTempString();
            }else{
                tempString += m1.getTempString();
                calculateFightOutcome();
            }
